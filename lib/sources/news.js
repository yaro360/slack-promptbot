...
async function fetchHackerNews(competitorNames, industryKeywords = []) {
  const signals = [];
  const searchTerms = [...competitorNames, ...industryKeywords];
  ...
  try {
    ...
    for (const story of stories.filter(Boolean)) {
      ...
      for (const term of searchTerms) {
        ...
        if (title.includes(lowerTerm) || url.includes(lowerTerm.replace(/\s+/g, ''))) {
          const ageHours = (Date.now() / 1000 - story.time) / 3600;
          if (ageHours <= 168) { // Last 7 days for weekly digest
            const isCompetitor = competitorNames.some(c => c.toLowerCase() === lowerTerm);
            signals.push({
              company: isCompetitor ? term : 'Industry',
              type: 'content',
              title: story.title,
              ...
              tag: isCompetitor ? 'content' : 'content',
              keyword: term
            });
  ...
  return signals.sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 15);
}
